"""
GDELT scraper that writes outputs + progress to S3.
"""
...
try:
    ...
    raise SystemExit("Missing dependency 'boto3'. Install with: pip install boto3") from e
...
# Finance-only URL path filters (domain -> list of required substrings).
# If list is non-empty, URL must contain at least one entry to be kept.
FINANCE_URL_FILTERS = {
    "cnn.com": [
        "/business/",
    ],
...
S3_PREFIX = os.getenv("S3_PREFIX", "gdelt/").lstrip("/")
...
def url_passes_finance_filter(domain: str, url: str | None) -> bool:
    if not url:
        return False
    rules = FINANCE_URL_FILTERS.get(domain, [])
    if not rules:
        return True
    url_l = url.lower()
    return any(r in url_l for r in rules)
...
# --- Progress tracking (S3) ---

def s3_key(*parts: str) -> str:
    return "/".join([S3_PREFIX.rstrip("/")] + [p.strip("/") for p in parts if p])


def load_progress(s3):
    key = s3_key("progress.json")
    try:
        ...
        log.warning(f"Failed to load progress from s3://{S3_BUCKET}/{key}: {e}")
        return {}


def save_progress(s3, progress: dict):
    key = s3_key("progress.json")
    s3.put_object(Bucket=S3_BUCKET, Key=key, Body=json.dumps(progress, indent=2).encode("utf-8"))
...
def mark_done(s3, progress: dict, domain: str, label: str):
    progress.setdefault(domain, {})[label] = True
    save_progress(s3, progress)
